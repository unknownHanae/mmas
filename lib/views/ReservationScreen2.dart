...
class ReservationScreen2 extends StatefulWidget {
  ...
  ReservationScreen2({Key? key, required this.reservations}) : super(key: key);

  @override
  State<ReservationScreen2> createState() => _ReservationState();
}

class _ReservationState extends State<ReservationScreen2> {
  ...
  void getContractsForClient(int client_id, context) {
    ...
    if(options.length == 1){
      ...
      BsSelectBoxOption one_select = options[0];
      _selectContract.setSelected(one_select);
      int id = one_select.getValue();
      Contrat contrat = contracts.where((el) => el.id_contrat == id).first;
      // getCoursByContrat(contrat);
    }
  }
  ...
  void fetchReservation() async {
    ...
    if(widget.reservations != null){
      data = widget.reservations!.map<Reservation>((e) => Reservation.fromJson(e)).toList();
      init_data = widget.reservations!.map<Reservation>((e) => Reservation.fromJson(e)).toList();
    }
    /*final response = await http.get(Uri.parse(
  ...
  Future<BsSelectBoxResponse> searchcontrat(Map<String, String> params) async {
    ...
    for(var v in contracts){
      if(v.abonnement!.toLowerCase().contains(params["searchValue"]!.toLowerCase())
          && v.id_etd! == _selectClient.getSelected()?.getValue()
      ){
        searched.add(
            BsSelectBoxOption(
                value: v.id_contrat,
                text: Text("Contrat ID ${v.id_contrat} - ${v.abonnement}-${v.type_contrat}")
            )
        );
      }
    }
  ...
  void delete(id) async {
    ...
    if (response.statusCode == 200) {
      // to edit
      setState(() {
        data = init_data.where((r) => r.id_reservation != id).toList();
        init_data = data;
      });
      //fetchReservation();
    } else {
  ...
  void getSeances(context, cour_id, {int? seance_id}) async {
    if(_selectClient.getSelected()?.getValue() != null && _selectContract.getSelected()?.getValue() != null){
      _selectSeances.clear();
      datePresenceController.text = "";
      date_presence = "";
      print("loading seances ...");
      var client_id = _selectClient.getSelected()?.getValue();
      Contrat contrat = contracts.where((c) => c.id_contrat == _selectContract.getSelected()?.getValue()).first;
      var url =  HOST+'/api/seance/?cour_id=${cour_id}&client_id=${client_id}';
      String cat_abn = contrat.cat_abn!.toLowerCase();
      if(cat_abn == "morning" || cat_abn == "relaxing"){
        url =  HOST+'/api/seance/?cour_id=${cour_id}&client_id=${client_id}&cat_abn=${cat_abn}';
      }
  ...
  // void getCoursByContrat(Contrat contrat){
  //   List<BsSelectBoxOption> options = [];
  //   cour.forEach((c) {
  //     if(c.genre == "Mixte"){
  //       options.add(BsSelectBoxOption(
  ...
  //       ));
  //     }else{
  //       if(c.genre!.toLowerCase().toString() == contrat.type!.toLowerCase().toString()){
  //         options.add(BsSelectBoxOption(
  //             value: c.id_cour,
  //             text: Text(" ${c.nom_cour}")
  //         ));
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        BsSelectBox(
                          ...
                          onChange: (v) {
                            int id_client = _selectClient.getSelected()?.getValue();
                            try{
                            ...
                            getContractsForClient(id_client, context);
                        ...
                        BsSelectBox(
                          ...
                          disabled: _selectContract.options.length == 1 ? true : false,
  ...
  void update(context, int id) async {
    ...
    )
    {
      ...
      if (response.statusCode == 200) {
        ...
        if(status == true){
          Navigator.pop(context,true);
          var index = init_data.indexWhere((r) => r.id_reservation == id);
          if(index != -1){
            Reservation resv = init_data[index];
            /**
             * Update values for :
             * "status": _selectStatus.getSelected()?.getValue(),
                "presence": _selectPrsesnce.getSelected()?.getValue(),
                "motif_annulation": moti_controller.text,
             */
            resv.status = _selectStatus.getSelected()?.getValue() == 1 ? true : false;
            resv.presence = _selectPrsesnce.getSelected()?.getValue() == 1 ? true : false;
            resv.motif_annulation = moti_controller.text;
            init_data[index] = resv;
            setState(() {
              init_data = init_data;
              data = init_data;
            });
                                  ...
                                  child: ListView(
                                      children:[
                                        ...
                                        Padding(
                                          ...
                                          child: Center(
                                              child: Text("aucun étudiant à afficher")),
                                        )
                                            :
                                        DataTable(
