...
class LoginCoachPage extends StatefulWidget {
  LoginCoachPage({Key? key, this.title}) : super(key: key);

  final String? title;

  @override
  _LoginPageCoachState createState() => _LoginPageCoachState();
}

class _LoginPageCoachState extends State<LoginCoachPage> {
        ...
        if (response.statusCode == 200) {
          ...
          if(body["success"] == true){
            ...
            try {
              ...
              prefs.setInt("id", data["id_coach"]);
              prefs.setString("name", "${data["nom_coach"]} ${data["prenom_coach"]}");
              ...
              context.read<AdminProvider>()
                  ...
                  Admin(
                      id: data["id_coach"],
                      name: "${data["nom_coach"]} ${data["prenom_coach"]}",
                      email: data["mail"],
                      //image: "${HOST}/media/${data["image"]}",
                      token: body["token"],
                      role: "coach"
                  )
              );
